...
struct AntiClassDump : public ModulePass {
  ...
  virtual bool doInitialization(Module &M) override {
    ...
    StructType *objc_property_attribute_t_type = reinterpret_cast<StructType *>(StructType::getTypeByName(
  ...
  bool runOnModule(Module &M) override {
    ...
    if (!M.getContext().supportsTypedPointers()) {
      errs() << "AntiClassDump isn't compatible with Opaqua Pointers, if you really need it, please add \" -mllvm -opaque-pointers=false -Xclang -no-opaque-pointers\" to the cflags for this project."  << "\n";
      return false;
    }
    GlobalVariable *OLCGV = M.getGlobalVariable("OBJC_LABEL_CLASS_$", true); // When Opaque Pointers is enabled, this line of code will crash LLVM, why ???
    if (OLCGV == NULL) {
